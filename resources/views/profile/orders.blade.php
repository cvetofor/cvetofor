                                    ...
                                    <div class="box">
                                        <div class="order">
                                                ...
                                                <div class="accordion__content" data-accordion-content="">
                                                    <div class="order__sellers-wrap">
                                                        ...
                                                        <div class="order__seller">
                                                            <div class="order__seller-heading"><span class="order__seller-title">{{ optional($order->market)->name ?? '...' }}</span>

                                                                @if(
                                                                $order->orderStatus?->code == \App\Models\OrderStatus::CANCELED_USER ||
                                                                $order->orderStatus?->code == \App\Models\OrderStatus::CANCELED)
                                                                    @php
                                                                        $class = 'canceled';
                                                                        $icon = '#icon-cross';
                                                                    @endphp
                                                                @elseif ($order->orderStatus?->code == \App\Models\OrderStatus::COMPLETE)
                                                                    @php
                                                                        $class = 'completed';
                                                                        $icon = '#icon-check';
                                                                    @endphp
                                                                @else
                                                                    ...
                                                                    @endphp
                                                                @endif
                                                                <div class="order__seller-status order__seller-status--{{ $class }}"><span class="order__seller-status__title">{{ $order->orderStatus?->title }}</span>
                                                            ...
                                                            <div class="order__seller-list">
                                                                ...
                                                                @endforeach
                                                                <div class="order__seller-list__item"><span>Доставка</span><span>@money($order->delivery->price) р.</span></div>
                                                            </div>
                                                        </div>
                                            ...
                                            <div class="order__bottom">
                                                ...
                                                @php
                                                    $isCanceled = $order->orderStatus?->code == \App\Models\OrderStatus::CANCELED_USER ||
                                                    $order->orderStatus?->code == \App\Models\OrderStatus::CANCELED;
                                                @endphp
                                                @if($order?->paymentStatus?->code === \App\Models\PaymentStatus::AWAIT && ! $isCanceled)
                                                    ...
                                                    @endif
                                                @endif
                                                <button class="link-button leave-review-button" data-modal-open="leave-review" onclick="review({{ $order->id }},'{{ optional($order->review)->description }}')"><span class="link-button__title">Отзыв</span>
